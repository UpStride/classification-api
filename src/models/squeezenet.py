...
class SqueezeNet(GenericModelBuilder):
  def model(self, x):
    ...
    x = self.layers.Conv2D(filters=self.label_dim, kernel_size=1, padding='valid', name='conv10')(x)
  ...
  def fire_module(self, fire_id, s1x1=16, e1x1=64, e3x3=64):
    ...
    x = self.layers.Concatenate(axis=3, name=s_id + 'concat')([expand1x1, expand3x3])
